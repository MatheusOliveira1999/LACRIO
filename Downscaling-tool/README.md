...
Um sistema avançado para downscaling estatístico de dados climáticos usando técnicas de Machine Learning, com processamento automático de dados meteorológicos via linha de comando.
...
- **Interface CLI**: Sistema de linha de comando com processamento automatizado
...
### 💻 Interface CLI
- **Processamento Automatizado**: Execução via linha de comando
- **Configuração Flexível**: Parâmetros ajustáveis via argumentos
- **Visualização Rica**: Gráficos automáticos salvos em arquivos
- **Resultados Organizados**: Relatórios e modelos salvos automaticamente
...
├── run_downscaling.py            # Script CLI principal
...
#### 3. Ver Ajuda
```bash
python run_downscaling.py --help
```

#### 4. Argumentos Disponíveis

| Argumento | Descrição | Obrigatório | Default |
|-----------|-----------|-------------|---------|
| `--era5` | Arquivo ERA5/ERA5-Land (CSV) | ✅ | - |
| `--station` | Dados da estação (CSV) | ✅ | - |
| `--dem` | Modelo digital elevação (NetCDF) | ✅ | - |
| `--lat` | Latitude da estação | ✅ | - |
| `--lon` | Longitude da estação | ✅ | - |
| `--variables` | Variáveis para downscaling | ❌ | temperature precipitation |
| `--models` | Modelos ML a usar | ❌ | 5 principais modelos |
| `--split-date` | Data divisão treino/teste | ❌ | 2018-01-01 |
| `--optimize` | Otimizar hiperparâmetros | ❌ | False |
| `--no-ensemble` | Não criar ensemble | ❌ | False |
| `--no-validation` | Desativar validação | ❌ | False |
...
### Exemplo 1: Downscaling Básico
```bash
# Usar dados da estação Llanganuco com modelos padrão
python run_downscaling.py \
    --era5 uploads/dados_era5_Llanganuco_LLANWX_processado.csv \
    --station uploads/Llanganuco_LLANWX_processado.csv \
    --dem uploads/Ranrahirca_static.nc \
    --lat -9.07  \
    --lon -77.65
```

### Exemplo 2: Downscaling Otimizado
```bash
# Com otimização de hiperparâmetros e modelos específicos
python run_downscaling.py \
    --era5 uploads/dados_era5_Cuchillacocha_processado.csv \
    --station uploads/Cuchillacocha_processado.csv \
    --dem uploads/Quilcayhuanca_static.nc \
    --lat -9.07 \
    --lon -77.65\
    --models RandomForest XGBoost MLP \
    --optimize \
    --output-dir resultados_cuchillacocha
```

### Exemplo 3: Apenas Temperatura
```bash
# Processar apenas temperatura com ensemble
python run_downscaling.py \
    --era5 dados_era5.csv \
    --station dados_estacao.csv \
    --dem modelo_elevacao.nc \
